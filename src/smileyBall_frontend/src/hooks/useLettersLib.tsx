...
import { useLetterOptions } from "@/hooks/useLetterOptions";
import { usePalette } from "@/hooks/usePalette";
import { fr } from "@/data/libs/ASCII_3x4_letters_builder_function";
...
const buildLibInit = async (libName, texture, callback) => {
  const loadLib = import(`@/data/libs`).then((mod) => mod[libName]);
  const loadTextureData = import(`@/data/textures`).then((mod) => mod[texture]);
  try {
